  ...
  constructor(container, client, options) {
    ...
    triggerTest(trigger, v) {
      ...
      if (trigger.startsWith("notifications.")) {
        ...
        var path = trigger.substring(0, p);
        ...
        console.log("Mean = %f, %f %f", mean, v, (mean + incr));
        if (this.values[path].length > 20) retval = (v > (mean + incr)); 
        this.values[path].push(v); if (this.values[path].length > 50) this.values[path].shift();
      }
      return(retval);
    }
}
