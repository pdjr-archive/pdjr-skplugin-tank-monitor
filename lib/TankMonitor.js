class TankMonitor {
  ...
  constructor(client, container, options) {
    ...
    this.signalkClient.getEndpoints(endpoints => {
      ...
      .forEach(endpoint => {
        ...
        if (!tweak.ignore) this.tanks.push(endpoint);
      });
    });
  ...
  tweak(path, tweaks = []) { 
    tweaks = tweaks.filter(tweak => ((tweak.path === undefined) || path.startsWith(tweak.path)));
    tweaks = tweaks.sort((a,b) => (a.path === undefined)?-1:((b.path === undefined)?+1:(a.path.length - b.path.length)));
    var retval = tweaks.reduce((a, v) => {
      if ((v.path === undefined) || path.startsWith(v.path)) {
        Object.keys(v).filter(k => (k != 'path')).forEach(k => {
          a[k] = v[k];
        });
      }
      return(a);
