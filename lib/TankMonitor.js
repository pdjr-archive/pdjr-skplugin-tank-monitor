class TankMonitor {
   ...
   *   to configure the TankMonitor. Defaults to
   *   "/signalk-tank-monitor/config.json")
   * container
  ...
  static create(config = {}) {
    config.config = (config.config)?config.config:"signalk-tank-monitor/config.json";
    ...
    try {
      ...
      signalkClient.waitForConnection().then(
        () => {
          fetch(config.config).then(
            (response) => {
              response.json().then(
                (options) => { new TankMonitor(signalkClient, config.container, options); },
                () => { new TankMonitor(signalkClient, config.container, { "tweaks": [] }); }
              );
            },
            () => { throw "HTTP server not responding"; }
          );
        },
  ...
  constructor(client, container, options) {
    ...
    this.tanks = new Set();
    this.values = {};
    this.popup = { container: null, image: null, navleft: null, navright: null, selection: [ 'day', 'week', 'month', 'year' ] };
  ...
  makeTankBar(tankpath) {
    ...
    if (tweak.log) tankBar.addEventListener("click", (e) => {
      while (this.popup.selection[0] != 'day') this.popup.selection.unshift(this.popup.selection.pop());
      this.popup.image.src = "graph/" + this.popup.selection[0] + ".svg";
